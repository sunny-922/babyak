...
export default function SignupPage() {
  ...
  const validate = () => {
    ...
    if (!form.school) e.school = '학교를 입력하세요.';
    if (!form.grade) e.grade = '학년을 입력하세요.';
    setErrors(e);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await signup({
        ...
        nickname: form.nickname, school: form.school, grade: form.grade,
      });
  ...
  const fields = [
    ...
    { key: 'school', label: '학교', placeholder: '학교명 입력', type: 'text' },
    { key: 'grade', label: '학년', placeholder: '예: 2학년', type: 'text' },
  ] as const;
